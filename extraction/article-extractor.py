...
"""
def write_content_to_file(filename, text, language, datatype, title=""):
...
def read_file_by_line(filename, languages):
    f = open(filename, "r")
    ne_filename_pair = open("ne_filename.txt", "a")
            ...
            for index, lang in enumerate(languages):
                if not titles[index] == "":
                    ...
                    if not article_resp == None:
                        ...
                        filepath = "data/%s/%s/%s.txt" %("articles", lang, "".join(titles[0].split()))
                        ne_filename_pair.write(str(i) + "\t" + filepath + "\n")
                        ne_filename_pair.flush()
                    backlink_resp = query_redirects(titles[index], lang)
                    if not backlink_resp == None:
                        write_content_to_file("".join(titles[0].split()), backlink_resp['backlinks'], lang, "backlinks", titles[0])
...
if __name__ == "__main__":
    ...
    print("generating")
    generate_name_backlinks(filename, languages)
